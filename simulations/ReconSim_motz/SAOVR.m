function [new_x, new_y, new_i, xremus, yremus, ststamp] = SAOVR(m, remus_speed, counter_in, x_range, y_range, x_remus_in, y_remus_in, detect_radius, refresh_time, change_x, change_y)
...
time_of_manuv =  600; %* 1.8; %m_length/remus_speed;
...
%base forward delta
 if change_y > 0 %1.8 %Going North
     t = [60, 180, 300, 60];
 elseif change_y < 0 %Going South
     t = [240, 0, 120, 240];
 elseif change_x > 0 %Going East
     t = [330, 90, 210, 330];
 end
...
%if change_y > 0 % 1.8 %Going North
%    t = [180, 60, 300, 180];
%elseif change_y < 0 % -1.8 %Going South
%    t = [0, 240, 120, 0];
%elseif change_x > 0 % 1.8 %Going East
%    t = [90, 330, 210, 90];
%end

%change_stop = [0, 100, 300, 500];
